...
- Human-visible gameplay review clips can now be captured with `npm run harness:capture:gameplay-segment` for any normal stage or Challenging Stage, persona/watch mode, seed, viewport, and duration. These clips persist under `reference-artifacts/analyses/gameplay-segment-captures/` and should be paired by identical seed/start/persona settings for before-after review.
...
Latest review clip: `reference-artifacts/analyses/gameplay-segment-captures/latest-stage11-challenge-improvement-professional.webm` captures a 30-second professional watch-mode review of Challenging Stage 3 / display Stage 11 with video and audio tracks validated in the generated JSON report.
