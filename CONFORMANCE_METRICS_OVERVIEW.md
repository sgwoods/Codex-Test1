...
Generated: `2026-05-03T16:29:42.050Z`
...
| Galaxy Guardians 0.1 dev preview | 7.6/10 | maturity 7.1/10; gate coverage 9.4/10; public readiness 4.4/10 | dev-preview-reference-conformance-model-not-public-release-score | Audio character and reference fit | reference-artifacts/analyses/galaxy-guardians-identity/reference-conformance-0.1.json |
...
| --- | --- | --- | --- | --- | --- |
| Current dev baseline | 8.8/10 | audio 6.1; movement 8.1; stage opening 8.5; challenge timing 8.4; shell integrity 9.2 | 7.6/10 | maturity 7.1; gate coverage 9.4; public readiness 4.4; audio fit 5.3 | Baseline for the next beta-candidate discussion. |
| `1.3` Fidelity and Trust | 9.0/10 | audio >= 7.2; movement >= 8.6; trust/fairness >= 9.3; shell integrity >= 9.4 | 7.6/10 | rack timing >= 6.5; movement pressure >= 6.7; visual identity >= 7.0; audio fit >= 5.3 | Aurora can move beta if the weakest feel gaps improve and Guardians stays dev-only but credible. |
...
xychart-beta
  ...
  line "Galaxy Guardians" [7.6, 7.6, 7.8, 8.2, 8.5, 9.0]
```
...
| --- | --- | --- | --- | --- | --- |
| Movement and pressure | 8.1/10 | 8.6/10 in `1.3`; 8.8/10 in `1.4` | 6.7/10 | browser-reviewed runtime tuning in `1.3`; 6.8/10 in `1.4` | This is the strongest direct feel signal during live play. |
| Audio identity / acoustic fit | 6.1/10 | 7.2/10 in `1.3`; 7.6/10 in `1.4` | 5.3/10 | isolated cue comparison in `1.3`; 6.8/10 in `1.5` | Audio is the weakest shared conformance area today. |
| Visual identity | 9.2/10 shell integrity; game sprites not separately scored in the roll-up | add a visible arcade-depth visual score in `1.4` | 7.0/10 | final sprite recognition in `1.3`; 7.6/10 in `1.4` | Guardians especially needs recognizably distinct alien silhouettes before beta-facing preview. |
| Stage / rack / wave timing | stage opening 8.5; challenge timing 8.4 | challenge and later-stage targets >= 8.6 in `1.4` | rack timing 6.5/10 | browser-reviewed rack timing in `1.3`; 7.2/10 in `1.4` | Timing separates authentic arcade pressure from approximate motion. |
...
| Evidence and replay durability | scorecard artifacts exist; video publishing is not yet a full product surface | replay/video evidence >= 8.8 in `1.5` | evidence durability 9.3 | final sprite/cue extraction durability >= 9.4 in `1.5` | Shared videos and source-controlled artifacts should become normal release evidence. |
...
| --- | --- | --- | --- | --- | --- |
| Reference source coverage | 8 | 9.2/10 | source-manifested-contact-sheets-waveforms-frame-proxies-object-track-proxies-runtime-comparison | Three Galaxian sources are committed with manifests, contact sheets, waveform windows, spectrograms, frame indexes, frame-motion proxies, a CPU-only connected-component/object-track proxy pass, runtime-vs-reference movement comparison, sprite crop extraction, and audio reference comparison. | Promote object/crop proxies into final sprite recognition and isolated acoustic cue matching. |
...
| Formation and rack timing | 12 | 6.5/10 | promoted-events-runtime-bands-frame-motion-proxy-and-object-track-proxy | Formation entry, settle, rack complete, and first-dive delay are modeled and gated. The frame and object proxies now cover opening rack entry and complete-rack reference windows. | The object proxy is still connected-component tracking rather than final sprite recognition, so runtime timing remains conservative until compared against instrumented runtime tracks. |
| Movement and pressure model | 14 | 6.7/10 | reference-window-inspired-runtime-contract-plus-runtime-vs-reference-track-comparison | Scout dives, flagship dives, escort joins, and wrap/return pressure are implemented and gated. Runtime movement was tuned against the object-track proxy, improving lower-field duration and lateral span while preserving first-dive, flagship, escort, and wrap events. | Local play still needs browser review and final sprite-recognition targets before making the preview public-facing. |
...
| Visual alien identity | 10 | 7/10 | game-owned-visual-catalog-readability-gate-frame-crops-object-proxies-and-sprite-extraction | Flagship, escort, scout, and player interceptor visuals are smaller and closer to Galaxian contact-sheet proportions while remaining distinct and gated. The sprite extraction pass now measures 27 rack/diver/player crops and confirms the runtime sprites cover the reference color families. | Sprites are still hand-authored preview approximations rather than extracted pixel-faithful reference assets. |
| Audio character and reference fit | 10 | 5.3/10 | internal-cue-shape-contract-plus-waveform-spectrogram-pcm-proxy | Runtime cue IDs and role-separated cue shapes are gated, and the audio comparison now analyzes 9 Galaxian waveform/spectrogram windows with PCM proxy metrics. | This is still the least reference-faithful Guardians area: isolated acoustic matching against Galaxian hardware cues is not yet automated. |
...
| Evidence durability | 10 | 9.3/10 | source-controlled-artifacts-and-harnesses | The reference profile, event log, identity artifacts, score/progression artifact, attract/score surface artifact, frame-motion artifact, object-track artifact, runtime-reference movement artifact, sprite extraction artifact, audio comparison artifact, and 0.1 gates are committed and rerunnable. | Add final sprite/cue extraction and browser-reviewed beta-candidate evidence. |
...
- final sprite recognition from representative Galaxian crop components
- Guardians attract and score-table pixel-layout comparison
- isolated acoustic cue comparison against waveform windows
