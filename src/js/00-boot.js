...
const ARCADE_MUSIC_PLAYLIST_OVERRIDES_PREF_KEY=`${STORAGE_PREFIX}ArcadeMusicPlaylistOverrides`;
...
const DEFAULT_GAME_SOUND_VOLUME=.20;
const DEFAULT_ARCADE_MUSIC_VOLUME=.80;
const ARCADE_MUSIC_BACKUP_PLAYLIST_ID='PLeDzKA-Cr6s-8b_YaTTleSX-h3X2QdQCo';
...
function readVolumePref(key,fallback=1){
 const raw=readPref(key);
 if(raw==null||raw==='')return fallback;
 const n=+raw;
 if(!Number.isFinite(n))return fallback;
 return clampVolumeValue(n>1?n/100:n,fallback);
}
function volumePercent(value){
 return Math.round(clampVolumeValue(value,0)*100);
}
function volumePercentText(value){
...
let arcadeMusicPlaylistOverrides=readJsonPref(ARCADE_MUSIC_PLAYLIST_OVERRIDES_PREF_KEY,{});
function arcadeMusicPlaylistOptions(){
...
function arcadeMusicPlaylistOverrideForGame(gameKey=''){
 ...
 if(!key)return '';
 const value=sanitizeArcadeMusicPlaylistId(arcadeMusicPlaylistOverrides?.[key]||'');
 return value||'';
}
function currentArcadeMusicPlaylistSelectValue(gameKey=''){
 const override=arcadeMusicPlaylistOverrideForGame(gameKey);
 return override||ARCADE_MUSIC_SELECTOR_DEFAULT;
}
function setArcadeMusicPlaylistOverrideForGame(gameKey='',value=''){
 const key=String(gameKey||'').trim();
 if(!key)return;
 const nextValue=value===ARCADE_MUSIC_SELECTOR_DEFAULT?'':sanitizeArcadeMusicPlaylistId(value);
 const nextOverrides=Object.assign({},arcadeMusicPlaylistOverrides||{});
 if(nextValue)nextOverrides[key]=nextValue;
 else delete nextOverrides[key];
 arcadeMusicPlaylistOverrides=nextOverrides;
 if(Object.keys(nextOverrides).length)writePref(ARCADE_MUSIC_PLAYLIST_OVERRIDES_PREF_KEY,JSON.stringify(nextOverrides));
...
const AC=()=>{
 ...
 sfx.bus.gain.value=audioMuted?0:gameSoundVolume;
...
const P={
 ship:{
  a:[[3,0],[4,0],[3,1],[4,1],[2,2],[3,2],[4,2],[5,2],[1,3],[2,3],[3,3],[4,3],[5,3],[6,3],[0,4],[1,4],[2,4],[3,4],[4,4],[5,4],[6,4],[7,4],[0,5],[1,5],[2,5],[5,5],[6,5],[7,5],[0,6],[1,6],[6,6],[7,6]],
  b:[[3,3],[4,3],[3,4],[4,4],[1,5],[6,5],[2,6],[3,6],[4,6],[5,6]],
  c:[[1,4],[6,4],[2,5],[5,5]]
 },
 bee:{
  a:[[0,1],[1,1],[6,1],[7,1],[0,2],[1,2],[6,2],[7,2],[0,3],[1,3],[6,3],[7,3],[1,4],[6,4]],
  b:[[2,0],[3,0],[4,0],[5,0],[2,1],[3,1],[4,1],[5,1],[2,2],[5,2],[2,3],[3,3],[4,3],[5,3],[3,4],[4,4],[3,5],[4,5]],
  c:[[3,1],[4,1],[2,3],[5,3]]
 },
 but:{
  a:[[0,0],[7,0],[0,1],[1,1],[6,1],[7,1],[1,2],[2,2],[5,2],[6,2],[2,3],[5,3],[1,4],[6,4],[1,5],[6,5],[0,6],[7,6]],
  b:[[3,0],[4,0],[3,1],[4,1],[2,2],[3,2],[4,2],[5,2],[3,3],[4,3],[3,4],[4,4],[3,5],[4,5]],
  c:[[3,2],[4,2],[2,4],[5,4],[3,6],[4,6]]
 },
 boss:{
  a:[[0,2],[1,2],[7,2],[8,2],[0,3],[1,3],[7,3],[8,3],[1,4],[2,4],[6,4],[7,4],[1,5],[2,5],[6,5],[7,5],[2,6],[6,6]],
  b:[[3,0],[4,0],[5,0],[2,1],[3,1],[4,1],[5,1],[6,1],[2,2],[3,2],[4,2],[5,2],[6,2],[3,3],[4,3],[5,3],[3,4],[4,4],[5,4],[4,5]],
  c:[[3,1],[5,1],[2,4],[6,4],[3,6],[5,6]]
 },
 dragonfly:{
  a:[[0,2],[1,1],[1,2],[1,3],[2,2],[6,2],[7,1],[7,2],[7,3],[8,2],[0,4],[1,4],[7,4],[8,4]],
  b:[[4,0],[3,1],[4,1],[5,1],[3,2],[4,2],[5,2],[3,3],[4,3],[5,3],[4,4],[4,5],[4,6]],
  c:[[3,2],[5,2],[2,4],[6,4],[4,6]]
 },
 mosquito:{
  a:[[0,3],[1,2],[1,3],[1,4],[2,3],[6,3],[7,2],[7,3],[7,4],[8,3],[2,5],[6,5]],
  b:[[4,0],[4,1],[3,2],[4,2],[5,2],[3,3],[4,3],[5,3],[3,4],[4,4],[5,4],[4,5],[4,6]],
  c:[[3,1],[5,1],[2,3],[6,3],[4,6]]
 },
 rogue:{
  a:[[3,0],[4,0],[3,1],[4,1],[2,2],[3,2],[4,2],[5,2],[1,3],[2,3],[3,3],[4,3],[5,3],[6,3],[0,4],[1,4],[2,4],[3,4],[4,4],[5,4],[6,4],[7,4],[0,5],[1,5],[2,5],[5,5],[6,5],[7,5],[0,6],[1,6],[6,6],[7,6]],
  b:[[3,3],[4,3],[3,4],[4,4],[1,5],[6,5],[2,6],[3,6],[4,6],[5,6]],
...
function currentStageCadenceSafe(){
 try{return currentGamePack()?.stageCadence||{}}
 catch{return {}}
}
function challengeStagesBefore(stage){
 const n=Math.max(1, stage|0);
 const cadence=currentStageCadenceSafe();
...
function challengeStageNumberForInternalStage(stage){
 const n=Math.max(1,stage|0);
 const cadence=currentStageCadenceSafe();
 const first=Math.max(1,cadence.challengeFirstStage||3);
 const every=Math.max(1,cadence.challengeEvery||4);
 if(n<first)return 0;
 return 1+Math.floor((n-first)/every);
}
function internalStageForChallengeStageNumber(challengeStage){
 const n=cl(+challengeStage||1,1,99)|0;
 const cadence=currentStageCadenceSafe();
 const first=Math.max(1,cadence.challengeFirstStage||3);
 const every=Math.max(1,cadence.challengeEvery||4);
 return first+(n-1)*every;
}
function challengeStageDisplayLabel(stageOrChallengeNumber,opts={}){
 const input=Math.max(1,+stageOrChallengeNumber||1)|0;
...
function syncChallengeStartControls(){
 ...
 const startKind=sanitizeStartKind(testStartKind?.value||cfg.startKind||(cfg.challenge?'challenge':'level'));
 const challengeStage=cl(+testChallengeStage?.value||cfg.challengeStage||DEFAULT_TEST_CFG.challengeStage,1,99)|0;
 if(testChallengeStageField)testChallengeStageField.classList.toggle('isInactive',startKind!=='challenge');
 if(testChallengeStage) {
...
function syncAudioMixControls(){
 ...
 const musicPct=volumePercent(arcadeMusicVolume);
 if(gameSoundVolumeControl){
  gameSoundVolumeControl.value=String(gamePct);
  gameSoundVolumeControl.setAttribute('aria-valuetext',volumePercentText(gameSoundVolume));
 }
 if(gameSoundVolumeValue)gameSoundVolumeValue.textContent=volumePercentText(gameSoundVolume);
 if(musicVolume){
  musicVolume.value=String(musicPct);
 ...
 if(arcadeMusicPlaylistSelect){
  const desiredOptions=arcadeMusicPlaylistOptions();
  const currentMarkup=Array.from(arcadeMusicPlaylistSelect.options).map(option=>`${option.value}:${option.textContent}`).join('|');
  const desiredMarkup=desiredOptions.map(option=>`${option.value}:${option.label}`).join('|');
  if(currentMarkup!==desiredMarkup){
   arcadeMusicPlaylistSelect.innerHTML=desiredOptions.map(option=>`<option value="${option.value}">${option.label}</option>`).join('');
  }
  arcadeMusicPlaylistSelect.value=currentArcadeMusicPlaylistSelectValue(playlistConfig?.gameKey||'');
 }
 if(playlistGameEl)playlistGameEl.textContent=playlistConfig?.gameTitle||'Current game';
 if(playlistSourceEl)playlistSourceEl.textContent=playlistConfig?.playlistSource==='selection'?'Developer override':playlistConfig?.playlistSource==='game'?'Game override':playlistConfig?.playlistSource==='harness'?'Harness':'Platform default';
 ...
 if(playlistOverrideEl)playlistOverrideEl.textContent=playlistConfig?.selectionPlaylistId||playlistConfig?.gamePlaylistId||'Inherits platform default';
 const debug=window.__platinumAudioDebug||window.__auroraAudioDebug;
 if(debug)debug.mix={
...
function logAudioMixSettingChange(setting='',source='developer_panel'){
 if(typeof logEvent!=='function')return;
 logEvent('audio_mix_changed',Object.assign({
  setting:String(setting||'audio_mix'),
  source:String(source||'developer_panel')
 },audioMixTelemetryBase()));
}
function setGameSoundVolume(value,opts={}){
 gameSoundVolume=clampVolumeValue(value,DEFAULT_GAME_SOUND_VOLUME);
 writePref(GAME_SOUND_VOLUME_PREF_KEY,gameSoundVolume.toFixed(3));
 if(sfx?.bus)sfx.bus.gain.value=audioMuted?0:gameSoundVolume;
 syncAudioMixControls();
 if(opts.log)logAudioMixSettingChange('game_sound_volume',opts.source||'developer_panel');
}
function setArcadeMusicVolume(value,opts={}){
 arcadeMusicVolume=clampVolumeValue(value,DEFAULT_ARCADE_MUSIC_VOLUME);
 writePref(ARCADE_MUSIC_VOLUME_PREF_KEY,arcadeMusicVolume.toFixed(3));
 if(typeof applyArcadeMusicVolume==='function')applyArcadeMusicVolume();
 syncAudioMixControls();
...
function conformanceDashboardUrl(){
 if(isLocalDashboardHost())return 'http://localhost:4312/local-dev/conformance-dashboard.html';
...
function syncConformanceDashboardUi(){
 ...
 if(conformanceDashboardHint){
  conformanceDashboardHint.textContent=local
   ? 'Open the full live dashboard at http://localhost:4312/local-dev/conformance-dashboard.html. Refresh it with npm run dev:conformance-dashboard during long cycles.'
...
function previewClassSuffix(value=''){
  return String(value||'')
    .trim()
...
function syncGamePreviewUi(){
 ...
 if(gamePreviewMission){
  ...
  const missionClass=previewClassSuffix(mission?.layoutClass||mission?.className||'');
  gamePreviewMission.className=missionClass?missionClass:'';
  ...
  const tableClass=previewClassSuffix(presentation.layoutClass||presentation.className||'');
  gamePreviewScoreTable.className=tableClass?tableClass:'';
	 ...
	 gamePreviewScoreTable.innerHTML=rows.length
	  ? [
	   ...
	   ...rows.flatMap(row=>[
	    `<span class="gamePreviewScoreLabel">${escapeGamePreviewHtml(row.scoreLabel||row.label||row.role||'Signal')}</span>`,
...
function showPlatformTrackMessage(title='',artist=''){
 const cleanTitle=String(title||'').trim();
 const cleanArtist=String(artist||'').trim();
 if(!platformTrackToast||!cleanTitle)return;
 if(platformTrackTitle)platformTrackTitle.textContent=cleanTitle;
 if(platformTrackArtist){
  platformTrackArtist.textContent=cleanArtist;
  platformTrackArtist.hidden=!cleanArtist;
 }
 platformTrackToast.classList.add('show');
 clearTimeout(platformTrackToastTimer);
 platformTrackToastTimer=setTimeout(()=>platformTrackToast.classList.remove('show'),4600);
}
...
if(arcadeMusicPlaylistSelect){
 arcadeMusicPlaylistSelect.addEventListener('change',()=>{
  const pack=typeof currentGamePack==='function'?currentGamePack():null;
