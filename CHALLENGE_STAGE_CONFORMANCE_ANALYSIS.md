...
- The single browser transfer proof for `stage3-semantic-fresh-g4-score-window-shape-peel-0.1` is now available at `reference-artifacts/analyses/reference-execution-runtime-expressibility/stage3-challenge1/latest-transfer-proof.json`, with SVG contact-sheet evidence at `reference-artifacts/analyses/reference-execution-runtime-expressibility/stage3-challenge1/latest-transfer-proof-contact-sheet.svg`. It now proves a motionSpec-backed path-length backend: `pathPlaybackScale` replaces the old unconsumed `groupReferencePaths[3].playbackScale` blocker for non-reference-path Stage 3 groups. The consumed controls are `pathPlaybackScale: 0.5`, `routeCurveY: 17.464`, and `routeOffsetX: 60`. Group 4's exit read moves from `center` to `right`, path length drops from `1.5589` to `1.1897`, upper-band share improves from `0.775` to `0.9474`, protected groups 1, 2, 3, and 5 preserve, and motion/profile, spacing, scoreable-route, no-combat, and no-shot/no-attack/no-loss guardrails pass. It is `runtime-source-attempt-ready` only, not a runtime keeper. Next Stage 3 work should attempt exactly one minimal source edit for these three controls, then run strict before/after evidence and accept or reject the player-facing keeper. Reference-path backing remains architecture debt.
