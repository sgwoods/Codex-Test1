// Main game source for Neo Galaga Tribute.
// This file is embedded into the served index.html by tools/build/build-index.js.
...
function shoot(){
 ...
 const shotXs=p.dual?[p.x-24,p.x+24]:[p.x];
...
function updateEnemy(e,dt,t,T,p){
 ...
 if(e.dive===5){
  ...
  if(!l){e.dive=1;e.low=0;e.lead=null;e.vx=rnd(26,-26);e.vy=104;return}
 ...
 if(e.cool<=0&&randUnit()<dt*diveRate&&S.att<attackCap&&S.recoverT<=0&&(!stageAttackGap||S.attackGapT<=0)&&!(S.stage===4&&S.stageClock<3.7)&&!(S.stage===5&&S.stageClock<1.9)){
  ...
  if(e.t==='boss'&&canCapture()&&randUnit()<T.capChance){e.dive=4;e.targetX=cl(p.x+rnd(34,-34),26,PLAY_W-26);e.targetY=138;e.vx=0;e.vy=128;e.shot=0;e.esc=0;logEnemyAttackStart(e,'capture',{targetX:+e.targetX.toFixed(2),targetY:e.targetY,scripted:0});}
  else{const steer=S.stage===4?.31:S.stage>=5?.48:.56,jitter=S.stage===4?20:S.stage>=5?32:38;e.dive=1;e.low=0;e.vx=(p.x-e.x)*steer+rnd(jitter,-jitter);e.vy=(S.stage===4?88:94)+rnd(S.stage===4?14:20);e.shot=e.t==='boss'?2:1;logEnemyAttackStart(e,'dive',{targetX:+p.x.toFixed(2),scripted:0});if(e.t==='boss')assignEscorts(e)}
