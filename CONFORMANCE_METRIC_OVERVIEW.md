...
| Guardians 0.1 preview | `7.4/10` reference conformance; `6.1/10` playtest weighted | Dev-preview | Score/progression, visible attract/score surfaces, frame/object proxies, runtime-vs-reference movement tuning, stage-rank pressure, sprite crop extraction, broad sprite-grid targets, audio waveform/spectrogram comparison, stricter audio cue targets, mixed-source cue candidates, and playtest-weighted gap scoring are now harnessed. |
...
| --- | ---: | --- | ---: | --- | --- |
| Current dev baseline | `8.8/10` | audio 6.1; movement 8.1; stage opening 8.5; challenge timing 8.4; shell integrity 9.2 | `7.4/10` reference; `6.1/10` playtest | maturity 6.5; gate coverage 9.2; public readiness 3.8; audio feel 4.7 | Baseline for the next beta-candidate discussion, now weighted by local playtest feel. |
| `1.3` Fidelity and Trust | `9.0/10` | audio >= 7.2; movement >= 8.6; trust/fairness >= 9.3; shell integrity >= 9.4 | `7.4/10` reference; `6.1/10` playtest | rack timing >= 6.2; movement pressure >= 6.2; visual identity >= 5.8; audio feel >= 4.7 | Aurora can move beta if the weakest feel gaps improve and Guardians stays dev-only but credible. |
...
xychart-beta
  ...
  line "Galaxy Guardians Reference" [7.4, 7.4, 7.8, 8.2, 8.5, 9.0]
  line "Galaxy Guardians Playtest" [6.1, 6.1, 7.0, 8.0, 8.4, 8.8]
```
...
| --- | ---: | ---: | ---: | ---: | --- |
| Movement and pressure | `8.1/10` | `8.6/10` in `1.3`; `8.8/10` in `1.4` | `6.2/10` playtest | browser-reviewed runtime tuning in `1.3`; `7.2/10` playtest in `1.4` | This is the strongest direct feel signal during live play. |
| Audio identity / acoustic fit | `6.1/10` | `7.2/10` in `1.3`; `7.6/10` in `1.4` | `4.7/10` playtest | manual cue labeling/export in `1.3`; `7.0/10` playtest in `1.4` | Audio is the weakest shared conformance area today. |
| Visual identity | `9.2/10` shell integrity; game sprites not separately scored in the roll-up | add a visible arcade-depth visual score in `1.4` | `5.8/10` playtest | tighter component sprite recognition in `1.3`; `7.0/10` playtest in `1.4` | Guardians especially needs recognizably distinct alien silhouettes before beta-facing preview. |
...
| Evidence and replay durability | scorecard artifacts exist; video publishing is not yet a full product surface | replay/video evidence >= 8.8 in `1.5` | evidence durability `9.5/10` | final sprite/cue extraction durability >= 9.5 in `1.5` | Shared videos and source-controlled artifacts should become normal release evidence. |
...
| Metric | Score | Read |
| --- | ---: | --- |
| Reference conformance | `7.4/10` | Useful development-preview baseline, with score/progression, attract/score surfaces, frame/object proxies, runtime movement tuning, stage-rank pressure, sprite extraction, sprite-grid targets, audio reference comparison, stricter audio cue targets, cue-candidate analysis, and playtest-weighted review now harnessed. |
| Playtest-weighted conformance | `6.1/10` | Stricter local feel score. Audio, pace, and graphic likeness are still below a compelling preview bar. |
| Reference maturity | `6.5/10` | Contact sheets, waveforms, spectrograms, frame indexes, frame/object metrics, runtime comparison, sprite crop metrics, broad sprite-grid targets, audio cue targets, audio PCM proxies, and mixed-source cue candidates exist; final extraction is still needed. |
| Implementation gate coverage | `9.2/10` | Most 0.1 runtime/visual/audio/platform gates are durable and rerunnable. |
| Public release readiness | `3.8/10` | The preview remains intentionally dev-only and should not be treated as public playable yet. |

### Guardians Reference Category Detail

| Category | Score | Status | Read |
| --- | ---: | --- | --- |
| Reference source coverage | `9.3/10` | Good | Three Galaxian sources are committed with manifests, contact sheets, waveforms, spectrograms, frame/object metrics, sprite crop extraction, broad sprite-grid targets, runtime comparison, audio comparison, and cue-candidate analysis. |
...
| Movement and pressure model | `6.2/10` | Improving but not convincing yet | Scout, flagship, escort, shots, wrap pressure, and bounded stage-rank pressure exist; the latest pass increased speed, density, and stage pressure, but still needs browser-reviewed reference feel. |
...
| Visual alien identity | `5.8/10` | Better separated, still partial | Sprites are smaller and distinct, 27 crop references feed a sprite extraction artifact, and broad sprite-grid targets now score flagship, escort, scout, and player silhouette similarity. They are still hand-authored approximations. |
| Audio character and reference fit | `4.8/10` | Weakest Guardians category | Runtime cue IDs are gated, shot/dive/hit/loss cues include stricter square/noise hardware targets, and 64 mixed-source cue candidates now exist; manual cue labeling/export is still pending. |
...
| Evidence durability | `9.5/10` | Good | Reference profile, event log, identity artifacts, movement tuning, stage-rank pressure, sprite extraction, sprite-grid targets, audio comparison, cue-candidate analysis, and gates are source-controlled and rerunnable. |
...
| Sprite grid targets | `sprite-grid-targets-0.1.json`, `analyze/check-galaxy-guardians-sprite-grid-targets.js` | Passes; broad crop-grid targets cover flagship, escort, scout, and player silhouettes with average similarity `0.622`. |
...
| Audio isolated cue candidates | `audio-isolated-cue-candidates-0.1.json`, `analyze/check-galaxy-guardians-isolated-cue-candidates.js` | Passes; 64 mixed-source candidate windows are grouped into shot/hit and dive/loss-like families for manual labeling/export. |
| Audio cue targets | `audio-cue-targets-0.1.json`, `check-galaxy-guardians-audio-cue-targets.js` | Passes; shot, enemy-shot, dive, hit, and loss cues now have stricter square/noise hardware-target gates. |
...
| Stage-rank pressure | `stage-rank-pressure-0.1.json`, `check-galaxy-guardians-stage-rank-pressure.js` | Passes; stage one is preserved while stages three and five add measured dive/shot/drift pressure. |
...
- Current Guardians gates are useful and green, but the playtest-weighted score
  is only `6.1/10`. The next step is manual Galaxian cue labeling/export,
  tighter component sprite-grid extraction, and a browser-reviewed pacing pass.
