...
| Guardians 0.1 preview | `7.6/10` reference conformance; `6.6/10` playtest weighted | Dev-preview | Score/progression, visible attract/score surfaces, frame/object proxies, runtime-vs-reference movement tuning, stage-rank pressure, component sprite targets, audio waveform/spectrogram comparison, labeled cue-window targets, stricter measured audio cue targets, and playtest-weighted gap scoring are now harnessed. |
...
| Audio identity / acoustic fit | `6.1/10` | `7.2/10` in `1.3`; `7.6/10` in `1.4` | `5.4/10` playtest | human-listened cue cleanup in `1.3`; `7.0/10` playtest in `1.4` | Audio is the weakest shared conformance area today. |
| Visual identity | `9.2/10` shell integrity; game sprites not separately scored in the roll-up | add a visible arcade-depth visual score in `1.4` | `6.7/10` playtest | browser-reviewed component-sprite polish in `1.3`; `7.0/10` playtest in `1.4` | Guardians especially needs recognizably distinct alien silhouettes before beta-facing preview. |
...
| Metric | Score | Read |
| --- | ---: | --- |
| Reference conformance | `7.6/10` | Useful development-preview baseline, with score/progression, attract/score surfaces, frame/object proxies, runtime movement tuning, stage-rank pressure, component sprite targets, audio reference comparison, measured audio cue targets, labeled cue-window previews, and playtest-weighted review now harnessed. |
| Playtest-weighted conformance | `6.6/10` | Stricter local feel score. Audio and motion feel are still below a compelling preview bar, but graphics are much closer. |
| Reference maturity | `7.0/10` | Contact sheets, waveforms, spectrograms, frame indexes, frame/object metrics, runtime comparison, sprite crop metrics, component sprite targets, measured audio cue targets, audio PCM proxies, and labeled cue candidates exist; human approval is still needed. |
| Implementation gate coverage | `9.5/10` | Most 0.1 runtime/visual/audio/platform gates are durable and rerunnable. |
| Public release readiness | `3.9/10` | The preview remains intentionally dev-only and should not be treated as public playable yet. |

### Guardians Reference Category Detail

| Category | Score | Status | Read |
| --- | ---: | --- | --- |
| Reference source coverage | `9.6/10` | Good | Three Galaxian sources are committed with manifests, contact sheets, waveforms, spectrograms, frame/object metrics, sprite crop extraction, component sprite targets, runtime comparison, audio comparison, cue-candidate analysis, and labeled cue-window previews. |
...
| Visual alien identity | `6.8/10` | Much closer, still needs browser review | Sprites are now tuned from component crop targets for flagship, escort, scout, and player ship while remaining pack-owned and separate from Aurora. |
| Audio character and reference fit | `5.5/10` | Weakest Guardians category | Runtime cue IDs are gated, shot/dive/hit/loss cues now use labeled cue-window target bands, and the refreshed audio comparison scores `8.3/10` overall with acoustic proxy fit at `5.3/10`; human listening is still pending. |
...
| Evidence durability | `9.7/10` | Good | Reference profile, event log, identity artifacts, movement tuning, stage-rank pressure, sprite extraction, sprite-grid/component targets, audio comparison, cue-candidate/labeled-cue artifacts, and gates are source-controlled and rerunnable. |
...
| Sprite grid targets | `sprite-grid-targets-0.1.json`, `analyze/check-galaxy-guardians-sprite-grid-targets.js` | Passes; grid targets now route through promoted component crops where available and currently score average similarity `1.0`. |
| Sprite component targets | `sprite-component-targets-0.1.json`, `promote/check-galaxy-guardians-sprite-component-targets.js` | Passes; promoted component crop previews now cover flagship, escort, scout, and player silhouettes and guide the runtime sprite rows. |
...
| Audio labeled cue targets | `audio-labeled-cue-targets-0.1.json`, `promote/check-galaxy-guardians-audio-cue-labels.js` | Passes; eight runtime cue families now have stable source timestamps plus waveform/spectrogram previews without raw audio export. |
...
- Current Guardians gates are useful and green, but the playtest-weighted score
  is only `6.6/10`. The next step is human listening on the labeled cue windows,
  browser review of the component-derived sprites, and a browser-reviewed pacing
  pass.
