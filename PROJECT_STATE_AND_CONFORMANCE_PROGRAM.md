...
| Galaxy Guardians | Second-game preview, branch-level playable-game push, and Galaxian-style ingestion proof | Preview metrics exist, public readiness is intentionally low, and the branch now treats it as a first-class conformance target with its own plan and aggregate process gate | Promote source-derived rack timing, dive paths, sprite/cue evidence, score/result identity, and playtest-weighted scoring through [GALAXY_GUARDIANS_FIRST_CLASS_CONFORMANCE_PLAN.md](GALAXY_GUARDIANS_FIRST_CLASS_CONFORMANCE_PLAN.md) before treating it as a serious public playable candidate |
