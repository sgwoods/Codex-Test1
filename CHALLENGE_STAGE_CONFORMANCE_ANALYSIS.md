...
- The first non-overwriting Stage 3 semantic batch is now available at `reference-artifacts/analyses/reference-execution-candidate-trials/stage3-challenge1/latest-batch.json`, with ranking calibration at `reference-artifacts/analyses/reference-execution-candidate-trials/stage3-challenge1/latest-ranking-calibration.json`. It generated `10` named candidates and kept no-combat, scoreable-route, spacing/readability, and safety guardrails intact. The calibrated ranker now separates strict object-track, path shape, path-length sanity, peel-off readability, semantic roles, upper-band scoreability, route learnability, no-combat grammar, spacing/readability, authority conflicts, and human-vs-CPU field-occupancy tension. `stage3-semantic-direct-lines-red-target-probe-0.1` is demoted to `metric-only-probe`, while `stage3-semantic-direct-lines-shape-peel-0.1` is the top `player-visible-semantic-lift`. Recommendation: generate one more small Stage 3 semantic batch using the calibrated ranker. Do not treat the RED, trial gate, or batch as runtime promotion authority by itself.
