...
| ---: | --- | --- | ---: | ---: | ---: | --- |
| 1 | Tighten audio reference segmentation and cue matching | audio | 2.1 | 0.064 | 2.9 | Tune the highest-risk runtime cue next: Challenge Perfect. Rerun audio comparison and event-gap analysis after the change. |
...
Audio remains the largest raw gap and top overall investment. Semantic audio measurement debt is now 0 attention rows, so the next audio task is Challenge Perfect. If the next cycle stays in level-arc instead, the immediate level-arc task is stage-1-baseline-missing-event-coverage.
