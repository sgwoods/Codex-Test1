...
const OUT_ROOT = path.join(ROOT, 'reference-artifacts', 'analyses', 'reference-execution-source-attempts', 'stage3-challenge1', 'group2-fast-lane');
const OUT_JSON = path.join(OUT_ROOT, 'latest-group2-fast-lane.json');
const OUT_MD = path.join(OUT_ROOT, 'latest-group2-fast-lane.md');
const OUT_SVG = path.join(OUT_ROOT, 'latest-group2-fast-lane-contact-sheet.svg');
...
function buildContactSheet(report, file){
  ...
  function plot(scenario, ox, title){
    ...
    const group2 = pathFor(scenario, 'group2', ox);
    if(group2) rows.push(`<path d="${group2}" fill="none" stroke="${colors[2]}" stroke-width="4" opacity="0.95"/>`);
    ...
    for(const groupIndex of [2, 4, 5]){
      ...
      if(pts.length){
        ...
        rows.push(`<circle cx="${round(first.x, 1)}" cy="${round(first.y, 1)}" r="${groupIndex === 2 ? 5 : 3}" fill="${colors[groupIndex]}" opacity="0.9"/>`);
  ...
  const body = [
    ...
    `<text x="52" y="22" fill="#dbeafe" font-size="16" font-family="Menlo, monospace">Stage 3 group 2 fast lane: ${esc(report.selectedCandidateId || report.verdict)}</text>`,
    plot(left, leftX, report.mode === 'source-attempt' ? 'Before group 2 source controls' : 'Baseline source'),
    ...
    '<text x="52" y="600">Group 2 highlighted green; group 4/5 keepers remain visible in pink/purple.</text>',
...
async function capture(){
  return withHarnessPage({ skipStart: true, stage: 3, ships: 3, challenge: false, seed: 9052 }, async ({ page }) => {
    return page.evaluate(({ variants, sampleStepS, sampleEndS, proofSeed, bounds, lowerFieldY, spacingGuard }) => {
      ...
      function applyGroup2MotionSpec(layout, motionSpec){
      ...
      function removeGroup2MotionSpec(layout){
        const next = clone(layout);
        if(Array.isArray(next.motionSpecGroups)) next.motionSpecGroups[1] = null;
        return next;
      }
      ...
      function summarizeScenario({ layoutOverride = null, variant = 'source' } = {}){
        ...
        return {
          ...
          centroidTraces,
          routeControlRead: {
            group2: samples.find(sample => sample.t >= 4.5)?.groups.find(group => group.groupIndex === 2)?.routeControls || [],
      ...
      const variantsOut = variants.map(variant => {
        const layoutOverride = applyGroup2MotionSpec(sourceLayout, variant.motionSpec);
        return {
      ...
      const beforeSourceEdit = summarizeScenario({ layoutOverride: removeGroup2MotionSpec(sourceLayout), variant: 'before-group2-source-controls' });
      return {
        sourceLayout,
        baseline,
...
function evaluateScenario({ baseline, scenario, variant = null, sourceAttempt = false }){
  const beforeGroup2 = baseline.groupMetrics.group2;
  const afterGroup2 = scenario.groupMetrics.group2;
  const groupDelta = vectorDelta(beforeGroup2, afterGroup2);
  const protectedGroups = [1, 3, 4, 5].map(groupIndex => {
  ...
  const centerEntryBefore = sideScore(beforeGroup2.entrySide, 'center');
  const centerEntryAfter = sideScore(afterGroup2.entrySide, 'center');
  const centerExitBefore = sideScore(beforeGroup2.exitSide, 'center');
  const centerExitAfter = sideScore(afterGroup2.exitSide, 'center');
  ...
  const pathLengthSane = Number.isFinite(+afterGroup2.pathLength)
    && afterGroup2.pathLength >= 0.55
    && afterGroup2.pathLength <= Math.max(1.25, (beforeGroup2.pathLength || 0) + 0.12);
  const targetRead = {
    group2: {
      before: beforeGroup2,
      after: afterGroup2,
      deltaFromBefore: groupDelta,
      entrySideBefore: beforeGroup2.entrySide,
      entrySideAfter: afterGroup2.entrySide,
      exitSideBefore: beforeGroup2.exitSide,
      exitSideAfter: afterGroup2.exitSide,
      ...
      upperBandScoreWindowPreserved: afterGroup2.upperBandShare >= Math.max(0, (beforeGroup2.upperBandShare || 0) - 0.05),
      lowerFieldOverstaySafe: afterGroup2.lowerFieldShare <= Math.min(0.46, (beforeGroup2.lowerFieldShare || 0) + 0.04),
  ...
  if(sourceAttempt && sourceConsumption.some(row => row.consumedByRuntime !== true || row.matchesExpected !== true)){
    failures.push({ category: 'source-control-not-consumed', read: 'One or more group 2 source controls did not appear on runtime enemies.' });
  }
  if(!targetRead.group2.browserVisiblePathMovement){
    failures.push({ category: 'no-visible-group2-movement', read: 'Group 2 did not move enough to count as a visible gameplay decision.' });
  }
  if(!targetRead.group2.columnCompactnessImproved){
    failures.push({ category: 'column-read-not-improved', read: 'The bee-column phrase did not become visibly tighter or more centered.' });
  }
  if(!targetRead.group2.verticalShapeControlled || !targetRead.group2.pathLengthSane){
    failures.push({ category: 'path-shape-regression', read: 'Group 2 vertical shape or path length moved outside the fast-lane sane range.' });
  }
  if(!targetRead.group2.upperBandScoreWindowPreserved || !targetRead.group2.lowerFieldOverstaySafe){
    failures.push({ category: 'score-window-regression', read: 'Group 2 upper-band scoreability or lower-field occupancy regressed.' });
  }
  if(!targetRead.group2.semanticColumnRolePreserved){
    failures.push({ category: 'semantic-role-regression', read: 'Group 2 no longer reads as a centered bee-column phrase.' });
  }
  if(!targetRead.group2.timingPreserved){
    failures.push({ category: 'phase-timing-regression', read: 'Group 2 visible timing drifted too far from the source phrase.' });
  }
  ...
  const score = (targetRead.group2.columnCompactnessImproved ? 0.26 : 0)
    + (targetRead.group2.verticalShapeControlled ? 0.14 : 0)
    + (targetRead.group2.pathLengthSane ? 0.14 : 0)
    + (targetRead.group2.semanticColumnRolePreserved ? 0.14 : 0)
    + (targetRead.group2.upperBandScoreWindowPreserved ? 0.14 : 0)
    + (targetRead.group2.lowerFieldOverstaySafe ? 0.1 : 0)
...
function markdown(report){
  ...
  return `# Stage 3 Group 2 Fast-Lane Report
...
- Entry side: ${report.group2Read?.group2?.entrySideBefore} -> ${report.group2Read?.group2?.entrySideAfter}
- Exit side: ${report.group2Read?.group2?.exitSideBefore} -> ${report.group2Read?.group2?.exitSideAfter}
- X range: ${report.group2Read?.group2?.before?.xRange} -> ${report.group2Read?.group2?.after?.xRange}
- Y range: ${report.group2Read?.group2?.before?.yRange} -> ${report.group2Read?.group2?.after?.yRange}
- Path length: ${report.group2Read?.group2?.before?.pathLength} -> ${report.group2Read?.group2?.after?.pathLength}
- Upper-band share: ${report.group2Read?.group2?.before?.upperBandShare} -> ${report.group2Read?.group2?.after?.upperBandShare}
- Lower-field share: ${report.group2Read?.group2?.before?.lowerFieldShare} -> ${report.group2Read?.group2?.after?.lowerFieldShare}
- Column compactness improved: ${report.group2Read?.group2?.columnCompactnessImproved}
- Semantic column role preserved: ${report.group2Read?.group2?.semanticColumnRolePreserved}
...
async function main(){
  ...
  const report = {
    ...
    scope: {
      ...
      touchedGroup: 2,
      protectedGroups: [1, 3, 4, 5],
      acceptedKeepersToPreserve: ['stage3-semantic-fresh-g4-score-window-shape-peel-0.1', 'stage3-g5-late-score-window-stretch-0.1']
    },
    variantsTested: selectedRows.rows.map(row => ({
      ...
      group2XRange: `${row.evaluation.targetRead.group2.before.xRange}->${row.evaluation.targetRead.group2.after.xRange}`,
      group2YRange: `${row.evaluation.targetRead.group2.before.yRange}->${row.evaluation.targetRead.group2.after.yRange}`,
      group2PathLength: `${row.evaluation.targetRead.group2.before.pathLength}->${row.evaluation.targetRead.group2.after.pathLength}`,
      exitSide: `${row.evaluation.targetRead.group2.exitSideBefore}->${row.evaluation.targetRead.group2.exitSideAfter}`,
      upperBandShare: `${row.evaluation.targetRead.group2.before.upperBandShare}->${row.evaluation.targetRead.group2.after.upperBandShare}`
    })),
    ...
    group2Read: evaluation?.targetRead || null,
    ...
    visualEvidence: {
      ...
      read: sourceAttempt
        ? 'Before-source-edit and after-source-edit browser traces are plotted with group 2 highlighted and group 4/group 5 keepers visible.'
        : 'Baseline and selected non-overwriting proof traces are plotted with group 2 highlighted and group 4/group 5 keepers visible.'
    },
    decision: {
