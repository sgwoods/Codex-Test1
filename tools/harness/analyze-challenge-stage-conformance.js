...
const TRIAL_ROOT = path.join(ANALYSES, 'challenge-stage-conformance-candidate-trials');
const CHALLENGE_STAGE_CANDIDATE_SWEEP = path.join(ANALYSES, 'challenge-stage-candidate-sweep', 'latest.json');
...
function candidateTrialConfig(){
  const candidateId = argValue('candidate', '');
  if(!candidateId) return null;
  const sweep = readJson(CHALLENGE_STAGE_CANDIDATE_SWEEP, {});
  const stage = +(argValue('stage', sweep.stage || '7') || 7);
  const candidate = (sweep.candidates || []).find(row => row.candidateId === candidateId)
    || (sweep.diagnostics?.deconflictTop || []).find(row => row.candidateId === candidateId)
    || (sweep.diagnostics?.routeAwareTop || []).find(row => row.candidateId === candidateId)
    || (sweep.diagnostics?.leastBunchedTop || []).find(row => row.candidateId === candidateId);
  if(!candidate){
    throw new Error(`Candidate ${candidateId} not found in reference-artifacts/analyses/challenge-stage-candidate-sweep/latest.json`);
  }
  if(!candidate.layout || typeof candidate.layout !== 'object'){
    throw new Error(`Candidate ${candidateId} does not include a layout override; rerun the sweep retaining layout data for diagnostic candidates.`);
  }
  return {
    stage,
    candidateId,
...
async function runtimeProbeForStage(stage, options = {}){
  const layoutOverride = options.layoutOverride && typeof options.layoutOverride === 'object'
    ? options.layoutOverride
    : null;
  return withHarnessPage({ stage, ships: 3, challenge: false, seed: 9100 + stage }, async ({ page }) => {
    return page.evaluate(({ stage, sampleTimes, layoutOverride }) => {
      const h = window.__galagaHarness__;
      h.setupChallengeMotionProfileTest({ stage, layoutOverride });
    ...
    }, { stage, sampleTimes: SAMPLE_TIMES, layoutOverride });
  });
}

async function collectRuntimeProbes(options = {}){
  const rows = [];
  for(const stage of CHALLENGE_STAGES){
    const layoutOverride = +options.candidateStage === +stage ? options.candidateLayout : null;
    const probe = await runtimeProbeForStage(stage, { layoutOverride });
    if(layoutOverride){
      probe.candidateTrial = {
        candidateId: options.candidateId || null,
        layoutOverrideApplied: true
      };
    }
...
function attachChallengeDiagrams(report, outDir, options = {}){
  ...
  for(const row of report.stageRows || []){
    ...
    if(updateLatest) writeText(latestPath, svg);
    row.trajectoryDiagram = updateLatest ? rel(latestPath) : rel(datedPath);
    ...
    if(updateLatest) writeText(objectTrackLatestPath, objectTrackSvg);
    row.objectTrackDiagram = updateLatest ? rel(objectTrackLatestPath) : rel(objectTrackDatedPath);
    row.evidence = Array.from(new Set([...(row.evidence || []), row.trajectoryDiagram, row.objectTrackDiagram]));
  }
  report.diagramArtifacts = {
    trajectoryDiagramDirectory: updateLatest ? rel(latestDir) : rel(outDir),
...
async function buildReport(options = {}){
  ...
  const runtimeProbes = await collectRuntimeProbes(options.candidateTrial ? {
    candidateStage: options.candidateTrial.stage,
    candidateLayout: options.candidateTrial.candidate.layout,
    candidateId: options.candidateTrial.candidateId
  } : {});
  ...
  if(options.candidateTrial){
    const row = rows.find(item => +item.stage === +options.candidateTrial.stage) || null;
    report.artifactType = 'challenge-stage-conformance-candidate-trial';
    report.candidateTrial = {
      stage: options.candidateTrial.stage,
      candidateId: options.candidateTrial.candidateId,
      sourceSweep: options.candidateTrial.sourceSweep,
      sweepExpectedScore10: options.candidateTrial.candidate.expectedScore10 ?? options.candidateTrial.candidate.expectedMatch?.score10 ?? null,
      sweepTargetVideoObjectFitScore10: options.candidateTrial.candidate.targetVideoObjectFitScore10 ?? options.candidateTrial.candidate.targetVideoObjectFit?.score10 ?? null,
      sweepHumanPerfectPotentialScore10: options.candidateTrial.candidate.humanPerfectPotentialScore10 ?? options.candidateTrial.candidate.humanPerfectPotential?.score10 ?? null,
      sweepHumanVisibleScore10: options.candidateTrial.candidate.humanVisibleGuardrails?.score10 ?? null,
      sweepBunchingRisk: options.candidateTrial.candidate.humanVisibleGuardrails?.bunchingRisk ?? null,
      sweepMagicAppearanceRisk: options.candidateTrial.candidate.humanVisibleGuardrails?.magicAppearanceRisk ?? null,
      trialStageConformanceScore10: row?.conformanceScore10 ?? null,
      trialStageMovementScore10: row?.movementConformanceScore10 ?? null,
      trialStageObjectScore10: row?.objectTrackProbe?.score10 ?? null,
      trialStageTargetFitScore10: row?.targetVideoObjectTrackFitScore10 ?? null,
      read: row
        ? `Candidate ${options.candidateTrial.candidateId} was applied only to Stage ${options.candidateTrial.stage} for strict analyzer review. Trial stage conformance ${row.conformanceScore10}/10, movement ${row.movementConformanceScore10}/10, object-track ${row.objectTrackProbe?.score10 ?? 'n/a'}/10, target-video fit ${row.targetVideoObjectTrackFitScore10 ?? 'n/a'}/10.`
        : 'Candidate trial row was not produced.'
    };
  }
  return report;
}
...
async function main(){
  const candidateTrial = candidateTrialConfig();
  const report = await buildReport({ candidateTrial });
  const safeCandidate = candidateTrial ? `-${candidateTrial.candidateId.replace(/[^a-z0-9._-]+/gi, '-')}` : '';
  const stamp = `${report.generatedAt.slice(0, 10)}-${report.commit}${safeCandidate}`;
  const outDir = candidateTrial ? path.join(TRIAL_ROOT, stamp) : path.join(OUT_ROOT, stamp);
  ...
  attachChallengeDiagrams(report, outDir, { updateLatest: !candidateTrial });
  ...
  console.log(JSON.stringify({
    ...
    candidateId: candidateTrial?.candidateId || null,
    score10: report.summary.score10,
    interestingFactorScore10: report.summary.interestingFactorScore10,
    report: rel(reportPath),
    latest: candidateTrial ? null : rel(latestPath),
    markdown: candidateTrial ? rel(readmePath) : rel(topLevelReportPath),
