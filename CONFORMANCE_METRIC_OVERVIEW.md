...
| Guardians 0.1 preview | `7.3/10` reference conformance; `5.9/10` playtest weighted | Dev-preview | Score/progression, visible attract/score surfaces, frame/object proxies, runtime-vs-reference movement tuning, stage-rank pressure, sprite crop extraction, audio waveform/spectrogram comparison, stricter audio cue targets, and playtest-weighted gap scoring are now harnessed. |
...
| Audio identity / acoustic fit | `6.1/10` | `7.2/10` in `1.3`; `7.6/10` in `1.4` | `4.4/10` playtest | isolated cue comparison in `1.3`; `7.0/10` playtest in `1.4` | Audio is the weakest shared conformance area today. |
...
| Metric | Score | Read |
| --- | ---: | --- |
| Reference conformance | `7.3/10` | Useful development-preview baseline, with score/progression, attract/score surfaces, frame/object proxies, runtime movement tuning, stage-rank pressure, sprite extraction, audio reference comparison, stricter audio cue targets, and playtest-weighted review now harnessed. |
| Playtest-weighted conformance | `5.9/10` | Stricter local feel score. Audio, pace, and graphic likeness are still below a compelling preview bar. |
| Reference maturity | `6.5/10` | Contact sheets, waveforms, spectrograms, frame indexes, frame/object metrics, runtime comparison, sprite crop metrics, audio cue targets, and audio PCM proxies exist; final extraction is still needed. |
...
| Audio character and reference fit | `4.4/10` | Weakest Guardians category | Runtime cue IDs are gated, and shot/dive/hit/loss cues now include stricter square/noise hardware targets, but isolated cue matching is still pending. |
...
| Evidence durability | `9.4/10` | Good | Reference profile, event log, identity artifacts, movement tuning, stage-rank pressure, sprite extraction, audio comparison, and gates are source-controlled and rerunnable. |
...
- Current Guardians gates are useful and green, but the playtest-weighted score
  is only `5.9/10`. The next step is isolated Galaxian cue matching, final
