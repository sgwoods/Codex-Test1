...
const OUT_ROOT = path.join(ROOT, 'reference-artifacts', 'analyses', 'reference-execution-source-attempts', 'stage3-challenge1', 'group5-fast-lane');
const OUT_JSON = path.join(OUT_ROOT, 'latest-group5-fast-lane.json');
const OUT_MD = path.join(OUT_ROOT, 'latest-group5-fast-lane.md');
const OUT_SVG = path.join(OUT_ROOT, 'latest-group5-fast-lane-contact-sheet.svg');
...
const VARIANTS = Object.freeze([
  Object.freeze({
    candidateId: 'stage3-g5-right-peel-read-0.1',
    transformId: 'group5-right-peel-read',
    read: 'Move the late closing line into a readable right peel while preserving the upper-band cleanup window.',
    motionSpec: Object.freeze({
      controls: Object.freeze({
        pathPlaybackScale: 1.05,
        routeOffsetX: 60,
        routeCurveY: 8
      })
    })
  ...
  Object.freeze({
    candidateId: 'stage3-g5-late-score-window-stretch-0.1',
    transformId: 'group5-late-score-window-stretch',
    read: 'Slightly advance the late phrase and add a longer rightward peel for stronger visible movement.',
    motionSpec: Object.freeze({
      controls: Object.freeze({
  ...
  Object.freeze({
    candidateId: 'stage3-g5-compact-right-cleanup-0.1',
    transformId: 'group5-compact-right-cleanup',
    read: 'Keep the late phrase compact but make the final cleanup peel visibly right-sided.',
    motionSpec: Object.freeze({
      controls: Object.freeze({
...
function sourceVariantFromLayout(layout){
  const group = Array.isArray(layout?.motionSpecGroups) ? layout.motionSpecGroups[4] : null;
...
function buildContactSheet(report, file){
  ...
  function plot(scenario, ox, title){
    ...
    if(group4) rows.push(`<path d="${group4}" fill="none" stroke="${colors[4]}" stroke-width="2.2" opacity="0.58"/>`);
    const group5 = pathFor(scenario, 'group5', ox);
    if(group5) rows.push(`<path d="${group5}" fill="none" stroke="${colors[5]}" stroke-width="4" opacity="0.95"/>`);
    for(const groupIndex of [4, 5]){
      ...
      if(pts.length){
        ...
        rows.push(`<circle cx="${round(first.x, 1)}" cy="${round(first.y, 1)}" r="${groupIndex === 5 ? 5 : 3}" fill="${colors[groupIndex]}" opacity="0.9"/>`);
  ...
  const body = [
    ...
    `<text x="52" y="22" fill="#dbeafe" font-size="16" font-family="Menlo, monospace">Stage 3 group 5 fast lane: ${esc(report.selectedCandidateId || report.verdict)}</text>`,
    plot(left, leftX, report.mode === 'source-attempt' ? 'Before group 5 source controls' : 'Baseline source'),
    ...
    '<text x="52" y="600">Group 5 highlighted purple; group 4 keeper remains visible in pink.</text>',
...
async function capture(){
  return withHarnessPage({ skipStart: true, stage: 3, ships: 3, challenge: false, seed: 9052 }, async ({ page }) => {
    return page.evaluate(({ variants, sampleStepS, sampleEndS, bounds, lowerFieldY, spacingGuard }) => {
      ...
      function applyGroup5MotionSpec(layout, motionSpec){
        const next = clone(layout);
        if(!Array.isArray(next.motionSpecGroups)) next.motionSpecGroups = [];
        const group = Object.assign({}, next.motionSpecGroups[4] || {});
        group.controls = Object.assign({}, group.controls || {}, motionSpec.controls || {});
        next.motionSpecGroups[4] = group;
        return next;
      }
      function removeGroup5MotionSpec(layout){
        const next = clone(layout);
        if(Array.isArray(next.motionSpecGroups)) next.motionSpecGroups[4] = null;
        return next;
      }
      ...
      const variantsOut = variants.map(variant => {
        const layoutOverride = applyGroup5MotionSpec(sourceLayout, variant.motionSpec);
        return {
      ...
      const beforeSourceEdit = summarizeScenario({ layoutOverride: removeGroup5MotionSpec(sourceLayout), variant: 'before-group5-source-controls' });
      return {
        sourceLayout,
        baseline,
...
function sourceControlRows({ scenario, variant, sourceAttempt }){
  const expected = Object.entries(variant.motionSpec.controls || {}).map(([key, value]) => ({
    runtimeField: `motionSpecGroups[4].controls.${key}`,
    expectedValue: value,
    enemyField: key
  }));
  const group = scenario.layoutSummary.motionSpecGroups?.[4] || {};
  const controls = group.controls || {};
  const routeReads = scenario.routeControlRead.group5 || [];
...
function evaluateScenario({ baseline, scenario, variant = null, sourceAttempt = false }){
  const beforeGroup5 = baseline.groupMetrics.group5;
  const afterGroup5 = scenario.groupMetrics.group5;
  const groupDelta = vectorDelta(beforeGroup5, afterGroup5);
  const protectedGroups = [1, 2, 3, 4].map(groupIndex => {
  ...
  const peelScoreBefore = sideScore(beforeGroup5.exitSide, 'right');
  const peelScoreAfter = sideScore(afterGroup5.exitSide, 'right');
  const targetRead = {
    group5: {
      before: beforeGroup5,
      after: afterGroup5,
      deltaFromBefore: groupDelta,
      exitSideBefore: beforeGroup5.exitSide,
      exitSideAfter: afterGroup5.exitSide,
      rightPeelScoreBefore: peelScoreBefore,
      rightPeelScoreAfter: peelScoreAfter,
      rightPeelImproved: peelScoreAfter > peelScoreBefore + 0.05,
      ...
      browserVisiblePeelMovement: Math.abs(groupDelta.exitCentroidDeltaPx || 0) >= 12 || afterGroup5.exitSide !== beforeGroup5.exitSide,
      pathLengthDirectionPass: Number.isFinite(+groupDelta.pathLengthDelta) && groupDelta.pathLengthDelta >= 0.03,
      upperBandScoreWindowPreserved: afterGroup5.upperBandShare >= Math.max(0, (beforeGroup5.upperBandShare || 0) - 0.08),
      lowerFieldOverstaySafe: afterGroup5.lowerFieldShare <= 0.18,
  ...
  if(sourceAttempt && sourceConsumption.some(row => row.consumedByRuntime !== true || row.matchesExpected !== true)){
    failures.push({ category: 'source-control-not-consumed', read: 'One or more group 5 source controls did not appear on runtime enemies.' });
  }
  if(!targetRead.group5.browserVisiblePathMovement){
    failures.push({ category: 'no-visible-group5-movement', read: 'Group 5 did not move enough to count as a visible gameplay decision.' });
  }
  if(!targetRead.group5.rightPeelImproved || !targetRead.group5.browserVisiblePeelMovement){
    failures.push({ category: 'peel-read-not-improved', read: 'The late closing phrase did not gain a readable right peel.' });
  }
  if(!targetRead.group5.pathLengthDirectionPass){
    failures.push({ category: 'path-length-not-improved', read: 'Group 5 path-length proxy did not increase from the overly flat source read.' });
  }
  if(!targetRead.group5.upperBandScoreWindowPreserved || !targetRead.group5.lowerFieldOverstaySafe){
    failures.push({ category: 'score-window-regression', read: 'Group 5 upper-band cleanup or lower-field safety regressed.' });
  }
  if(!targetRead.group5.timingPreserved){
    failures.push({ category: 'phase-timing-regression', read: 'Group 5 visible timing drifted too far from the source phrase.' });
  }
  ...
  const score = (targetRead.group5.rightPeelImproved ? 0.24 : 0)
    + (targetRead.group5.browserVisiblePeelMovement ? 0.16 : 0)
    + (targetRead.group5.pathLengthDirectionPass ? 0.18 : 0)
    + (targetRead.group5.upperBandScoreWindowPreserved ? 0.14 : 0)
    + (targetRead.group5.lowerFieldOverstaySafe ? 0.1 : 0)
...
function selectBestVariant(captureData){
  ...
  return { rows, selected: pool[0] || null, passing };
}
...
function markdown(report){
  ...
  return `# Stage 3 Group 5 Fast-Lane Report
...
- Exit side: ${report.group5Read?.group5?.exitSideBefore} -> ${report.group5Read?.group5?.exitSideAfter}
- Path length: ${report.group5Read?.group5?.before?.pathLength} -> ${report.group5Read?.group5?.after?.pathLength}
- Upper-band share: ${report.group5Read?.group5?.before?.upperBandShare} -> ${report.group5Read?.group5?.after?.upperBandShare}
- Lower-field share: ${report.group5Read?.group5?.before?.lowerFieldShare} -> ${report.group5Read?.group5?.after?.lowerFieldShare}
- Right peel improved: ${report.group5Read?.group5?.rightPeelImproved}
...
async function main(){
  ...
  const report = {
    schemaVersion: 1,
    artifactType: 'stage3-group5-fast-lane-report',
    generatedAt: new Date().toISOString(),
    generatedBy: 'tools/harness/analyze-stage3-group5-fast-lane.js',
    ...
    variantsTested: selectedRows.rows.map(row => ({
      ...
      group5PathLength: `${row.evaluation.targetRead.group5.before.pathLength}->${row.evaluation.targetRead.group5.after.pathLength}`,
      exitSide: `${row.evaluation.targetRead.group5.exitSideBefore}->${row.evaluation.targetRead.group5.exitSideAfter}`,
      upperBandShare: `${row.evaluation.targetRead.group5.before.upperBandShare}->${row.evaluation.targetRead.group5.after.upperBandShare}`
    })),
    ...
    group5Read: evaluation?.targetRead || null,
    ...
    visualEvidence: {
      ...
      read: sourceAttempt
        ? 'Before-source-edit and after-source-edit browser traces are plotted with group 5 highlighted and group 4 keeper visible.'
        : 'Baseline and selected non-overwriting proof traces are plotted with group 5 highlighted and group 4 keeper visible.'
    },
    decision: {
      sourceEditKept: verdict === 'dev-visible-gameplay-keeper',
      read: verdict === 'dev-visible-gameplay-keeper'
        ? 'Group 5 source controls reproduced the fast-lane proof and are accepted as another dev-visible gameplay keeper only.'
        : (verdict === 'transfer-proof-ready'
          ? 'Exactly one group 5 non-overwriting proof candidate is ready for exactly one minimal source edit.'
          : 'No unique group 5 fast-lane candidate passed quickly; stop without broadening the search.')
    },
    summary: {
      selectedCandidateId: selected?.candidateId || null,
      verdict,
