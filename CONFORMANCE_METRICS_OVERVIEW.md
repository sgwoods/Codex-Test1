...
Generated: `2026-05-03T15:16:23.603Z`
...
| Galaxy Guardians 0.1 dev preview | 7.4/10 | maturity 6.7/10; gate coverage 9.2/10; public readiness 4.1/10 | dev-preview-reference-conformance-model-not-public-release-score | Audio character and reference fit | reference-artifacts/analyses/galaxy-guardians-identity/reference-conformance-0.1.json |
...
| --- | --- | --- | --- | --- | --- |
| Reference source coverage | 8 | 9.1/10 | source-manifested-contact-sheets-waveforms-frame-proxies-object-track-proxies | Three Galaxian sources are committed with manifests, contact sheets, waveform windows, spectrograms, frame indexes, frame-motion proxies, and a CPU-only connected-component/object-track proxy pass. | Promote the object-track proxies into runtime-vs-reference comparison and measured sprite extraction. |
...
| Formation and rack timing | 12 | 6.4/10 | promoted-events-runtime-bands-frame-motion-proxy-and-object-track-proxy | Formation entry, settle, rack complete, and first-dive delay are modeled and gated. The frame and object proxies now cover opening rack entry and complete-rack reference windows. | The object proxy is still connected-component tracking rather than final sprite recognition, so runtime timing remains conservative until compared against instrumented runtime tracks. |
| Movement and pressure model | 14 | 6.1/10 | reference-window-inspired-runtime-contract-plus-frame-and-object-proxies | Scout dives, flagship dives, escort joins, and wrap/return pressure are implemented and gated. The object-track proxy now extracts lower-field candidate tracks from dive, flagship, and wrap windows, giving the next runtime tuning pass concrete x/y span and duration bands. | Local play still reads approximate; dive curves and wave-to-wave pacing need runtime-vs-reference track comparison before major constant changes. |
...
| Visual alien identity | 10 | 6.7/10 | game-owned-visual-catalog-readability-gate-frame-crops-and-object-proxies | Flagship, escort, scout, and player interceptor visuals are smaller and closer to Galaxian contact-sheet proportions while remaining distinct and gated. The frame/object passes now preserve rack/diver/player crop context and object-cluster evidence needed for extraction. | Sprites are still hand-authored preview approximations rather than extracted pixel-faithful reference assets. |
| Audio character and reference fit | 10 | 4.8/10 | internal-cue-shape-contract-plus-waveform-spectrogram-windows | Runtime cue IDs and role-separated cue shapes are gated, and the selected Galaxian frame windows now preserve waveform and spectrogram evidence alongside the motion proxy. | This is still the least reference-faithful Guardians area: acoustic comparison against Galaxian footage is not yet automated. |
...
| Evidence durability | 10 | 9.1/10 | source-controlled-artifacts-and-harnesses | The reference profile, event log, identity artifacts, score/progression artifact, attract/score surface artifact, frame-motion artifact, object-track artifact, and 0.1 gates are committed and rerunnable. | Add runtime-vs-reference track comparison and generated tuning bands once the metrics are less proxy-driven. |
...
- runtime-vs-reference rack-entry track comparison from matt-hawkins-arcade-intro
- runtime-vs-reference dive-path comparison from arcades-lounge-level-5 and nenriki-15-wave-session
