...
Generated: `2026-05-17T02:19:01.762Z`
...
| --- | --- | --- | --- | --- | --- |
| Aurora Galactica current dev line | 9.2/10 | strongest Shot and hit responsiveness 10/10; weakest Audio identity and cue alignment 7.2/10; broad alien/challenge variation 8.3/10; dedicated challenge-stage conformance 6.1/10; audio contract readiness 9.24/10 | release-quality conformance score plus audio cue-contract read | Audio identity and cue alignment (7.2/10) | reference-artifacts/analyses/quality-conformance/2026-05-17-612f086f/report.json; reference-artifacts/analyses/aurora-audio-cue-contracts/latest.json; reference-artifacts/analyses/challenge-stage-conformance/latest.json |
...
| --- | --- | --- | --- | --- | --- |
| Current dev baseline | 9.2/10 | audio 7.2; movement 9.8; boss/formation 9.4; broad alien/challenge 8.3; dedicated challenge-stage 6.1; stage opening 8.5; challenge timing 10; shell integrity 9.2 | 7.6/10 reference; 6.9/10 playtest | maturity 7.2; gate coverage 9.6; public readiness 4.2; audio feel 6.4 | Baseline for the next beta-candidate discussion, now weighted by local playtest feel. |
...
xychart-beta
  ...
  line "Aurora Galactica" [9.2, 9.0, 9.2, 9.3, 9.4, 9.5]
...
| --- | --- | --- | --- | --- | --- |
| Movement and pressure | 9.8/10 | 8.6/10 in `1.3`; 8.8/10 in `1.4` | 6.2/10 playtest; 6.2/10 reference category | browser-reviewed runtime tuning in `1.3`; 7.2/10 playtest in `1.4` | This is the strongest direct feel signal during live play. |
| Audio identity / acoustic fit | 7.2/10 | 7.2/10 in `1.3`; 7.6/10 in `1.4` | 6.4/10 playtest; 6.4/10 reference category | human-listened cue cleanup in `1.3`; 7.0/10 playtest in `1.4` | Audio is the weakest shared conformance area today. |
...
| --- | --- | --- | --- |
| Player movement conformance | 9.8/10 | player-movement report | Current movement scored 9.8/10 against the control-principles profile, versus 9.8/10 for the shipped local baseline. |
| Shot and hit responsiveness | 10/10 | close-shot-hit, movement fire window | Close-shot responsiveness passed, and movement-fire post-shot travel was 64.66, with shot delay -8ms. |
...
| Audio identity and cue alignment | 7.2/10 | audio-cue-alignment correspondence, aurora-audio-theme-comparison, galaga-audio-overlap, aurora-audio-event-gap semantic read, audio-cue-contracts, audio-promotion-precheck | Audio score blends cue identity, active reference similarity, reference-window precision, overlap timing, cue alignment, semantic event mapping, and acoustic event-gap severity. 0/21 reference windows still need tighter segmentation; 17 candidate subwindows were found; semantic event score is 9.78/10, acoustic event score is 6.32/10, and highest segment risk is gameOver onset. Cue-contract readiness is 9.24/10; latest contract next step: Run a contract-aware candidate pass for rescueJoin, then rerun promotion precheck and live validation. |
...
| Dedicated challenge-stage set-piece conformance | 6.1/10; interesting factor 6/10 | reference-artifacts/analyses/challenge-stage-conformance/latest.json, CHALLENGE_STAGE_CONFORMANCE_ANALYSIS.md, challenge-stage correspondence and motion-profile harnesses | challenge stages are still not authored enough as memorable Galaga-like set pieces: 1 sampled stage(s) still best-match the same Galaga challenge-2 reference, stage 3 now lands on the first-challenge bee-line reference but needs stronger trajectory precision, active sprite-motion novelty is unscored, and stage 19 lacks reference grounding. Player meaning: A player should experience challenging stages as safe but tense score exhibitions with memorable entry routes and alien novelty. Aurora currently has the safety rule, but the visual choreography is not yet distinctive enough. Next recommended step: Separate rule conformance from interest conformance in challenge metrics. |
...
- browser-reviewed stage-five-and-beyond motion/fairness pass with persona review
