...
Generated: 2026-06-09T01:47:06.471Z
Commit: 30fdad3c
...
**Movement read:** Strict movement score 4.2/10 against challenge-1-arrival-group-1: y-range fit 1, path-length fit 1, turn fit 1, object-track fit 0.25. Current probes now include runtime object tracks compared against CPU-extracted Galaga target-video object tracks where available. Legacy broad vector best-match was 8.5/10 against challenge-1-arrival-group-1; xRange 0.9883, yRange 0.7059, pathLength 0.807.
...
**Target contract read:** Target contract fit is 6.8/10 for Challenging Stage 1 (Levels 3-4): group count 1, runtime path-family order 1, declared path-family order n/a, type order 1, family order 1, object-track 0.25 via direct target-video tracks. This is a group/object-track read, not frame-perfect sprite identity recognition.

**Shot-opportunity read:** Shot-opportunity probe found scoreable targets in 61 sampled windows; 82% had a lane with 2+ targets, lane diversity was 1, and center-lane bias was 0.08.

**Safety rule:** enemy shots 0, attack starts 0, ship losses 0.

**Keeper-cycle note:** The Stage 3 source attempt for `stage3-semantic-fresh-g4-score-window-shape-peel-0.1` is accepted as a `/dev` visible gameplay keeper, not beta justification by itself. The source change is limited to group 4 motionSpec controls: `pathPlaybackScale: 0.5`, `routeCurveY: 17.464`, and `routeOffsetX: 60`. Source evidence at `reference-artifacts/analyses/reference-execution-source-attempts/stage3-challenge1/latest-source-attempt.json` reproduces the proof-backed read: group 4 exit `center` -> `right`, path length `1.5589` -> `1.1897`, upper-band share `0.775` -> `0.9474`, protected groups 1, 2, 3, and 5 preserve, and motion/profile, spacing, scoreable-route, no-combat, zero-shot, zero-attack, zero-ship-loss, and zero-challenge-contact guardrails pass.
...
**Next actions:**
- Preserve the accepted group 4 source keeper, then target the next focused Stage 3 movement gap: group 1 direct-line/object-track precision and first-visible lane/rack-slot timing against challenge-1 arrival labels.
- Add the same source-attempt before/after contact-sheet and strict guardrail pattern to the next candidate, with no new batch unless the RED/trial language cannot express the target gap.
